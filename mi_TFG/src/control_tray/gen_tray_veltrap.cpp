...
class Clase_Servicio_Accion : public rclcpp::Node
{
    public:
        ...
        Clase_Servicio_Accion() : rclcpp::Node("gen_tray_veltrap")
        {
    ...
    private:
        ...
        // Estructura para obtener los coeficientes de cada trayectoria
        struct trap_vel
        {
            double p0, pf;
            double v0, vf;
            ...
            double t1, tc, t2;  // duraciones
            double p1, p2;      // posiciones en cambios

            bool triangular;
        };

        double proyeccion(double velocidad_abs, char eje, double xf, double yf, double zf)
        {
            double x0 = last_pose_.pose.position.x;
            double y0 = last_pose_.pose.position.y;
            double z0 = last_pose_.pose.position.z;

            Eigen::Vector3d dp(xf - x0, yf - y0, zf - z0);
            double L = dp.norm();

            Eigen::Vector3d u = dp / L;

            double comp = 0.0;
            if (eje == 'X') comp = std::abs(u.x());
            else if (eje == 'Y') comp = std::abs(u.y());
            else if (eje == 'Z') comp = std::abs(u.z());
        ...
        trap_vel calcular_coeficientes(double q0, double qf, double v0, double vf, double vmax)
        {
            trap_vel c;
            ...
            double x1 = (vmax*vmax-v0*v0)/(2*c.a);
            double x2 = (vmax*vmax-vf*vf)/(2*c.a);

            if (x1 + x2 >= std::abs(qf - q0))     // Triangular
            {
                c.triangular = true;
                c.v_max = std::sqrt(std::max(0.0, c.a*std::abs(qf - q0) + 0.5*(std::abs(v0)*std::abs(v0) + std::abs(vf)*std::abs(vf))));
            ...
            else                        // Trapezoidal
            {
                ...
                c.t1 = (c.v_max - std::abs(v0))/c.a;
                c.tc = (std::abs(qf - q0) - x1 - x2) / c.v_max;
                c.t2 = (c.v_max - std::abs(vf))/c.a;
        ...
        void execute(const std::shared_ptr<GoalHandleTrayAction> goal_handle)
        {
            ...
            if (!eliminado)
            {
                ...
                trap_vel cx = calcular_coeficientes(x0, xf, vx0, vxf, proyeccion(v_max_lin, 'X', xf, yf, zf));
                trap_vel cy = calcular_coeficientes(y0, yf, vy0, vyf, proyeccion(v_max_lin, 'Y', xf, yf, zf));
                trap_vel cz = calcular_coeficientes(z0, zf, vz0, vzf, proyeccion(v_max_lin, 'Z', xf, yf, zf));
                trap_vel cyaw = calcular_coeficientes(yaw0, yawf, vyaw0, vyawf, v_max_ang);

                t_total = std::max({cx.tf, cy.tf, cz.tf, cyaw.tf});
                ...
                while (rclcpp::ok() && !eliminado && t <= t_total) 
                {
                    ...
                    if (!goal_handle->is_canceling())
                    {
                        ...
                        std_msgs::msg::Float64MultiArray msg_x = eval_trap(cx, t);
                        std_msgs::msg::Float64MultiArray msg_y = eval_trap(cy, t);
                        std_msgs::msg::Float64MultiArray msg_z = eval_trap(cz, t);
                        std_msgs::msg::Float64MultiArray msg_yaw = eval_trap(cyaw, t);
